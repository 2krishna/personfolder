...
class ResidentsViewSet(viewsets.ViewSet):
    def list(self,request):
        ...
        if type:
            try:
                ...
                paginator = Paginator(residentsdata,10)
                ...
                for residentsdata in residentsdata:
                    residentsdatasearchtypelist.append({
                        'full_name':residentsdata.residents.name,
                        ...
                        'tower':residentsdata.tower,
                    })
                
                return Response({'response':residentsdatasearchtypelist,'total_no_pages':total_page,'message':True,'status':status.HTTP_200_OK})
            except Exception as error:
                traceback.print_exc()
            ...
            if search:
                try:
                    ...
                    residentsdata=Residents.objects.filter(Q(residents__name__icontains=search))
                    page = request.GET.get('page')
                ...
                try:
                    ...
                    for residents_data in users:
                        residents_data_list.append({
                            ...
                            'full_name':residents_data.residents.name,
                    ...
                    traceback.print_exc()
                    return Response({'response':str(error),'status':status.HTTP_200_OK,'success':False})
    ...
    def create(self,request):
        try:
            ...
            residents_data.tower=request.data.get('tower')
            residents_data.save()
    ...
    def update(self,request,pk=None):
        try:
            ...
            tower=request.data.get('tower')
            if not tower:
                return Response({'response':'please enter the tower ','success':True,'status':status.HTTP_200_OK})
            residentsdata.tower=tower
            residentsdata.save()
        ...
        try:
            ...
            for attendancedata in attendancedata:
                attendancedata_list.append({
                    ...
                    'service':attendancedata.employee.service_provider,
                    'present':attendancedata.attandance
                })
            ...
            dashdata=[{
                ...
                "all_employee":attendancedata_list,
    ...
    def list(self,request):
        try:
            ...
            if month and name:
                ...
                pass
                attendancedata=Attendance.objects.filter(Q(employee__name__icontains=search))
                page = request.GET.get('page')
                ...
                attendancedata=Attendance.objects.filter(Q(date_fields__month=month))
                page = request.GET.get('page')
...
class Request_backupViewSet(viewsets.ViewSet):
    def list(self,request):
        try:
            ...
            if search:
            ...
            elif type and service:
                residentdata_list=[]
                residentdata=Request_backup.objects.filter(resident_request__type=type,
                emp_request__service_provider=service)
                # residentdata=Request_backup.objects.filter(Q(resident_request__type=type)&Q(emp_request__service_provider=service))
                page = request.GET.get('page')
                ...
                return Response({'response':requestdata_list,'total_pages':total_page,'success':True,'status':status.HTTP_200_OK})
        except Exception as error:
            traceback.print_exc()
            return Response({'response':str(error),'success':False,'status':status.HTTP_200_OK})
    def create(self,request):
    ...
    def list(self,request):
        try:
            ...
            if type:
                ...
                return Response({'response':residentdata_list,'total_page':total_page,'success':True,'status':status.HTTP_200_OK})
            elif employee:
            ...
            #     return Response({'response':attendancedata_list,'total_present_day':total_present_day,'total_present':total_present,'total_absent':total_absent})
                ...
                return Response({'response':requestbackup_data_list,'total_payment':total_payment,'total_page':total_page,'message':True,'status':status.HTTP_200_OK})
            elif analysis:
                ...
                return Response({'response':'please enter the filter query','success':True,'status':status.HTTP_200_OK})
        except Exception as error:
            traceback.print_exc()
            return Response({'response':str(error),'success':False,'status':status.HTTP_200_OK})
